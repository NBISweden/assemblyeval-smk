...
rule kraken2_gather_results:
    ...
    input:
        output = expand(__INTERIM__ / "kraken2/{{assembly}}/{{db}}.{{length}}.{partition}.output.txt.gz", partition=range(0, config["kraken2"]["npartitions"])),
        unclassified = expand(__INTERIM__ / "kraken2/{{assembly}}/{{db}}.{{length}}.{partition}.unclassified.fasta.gz", partition=range(0, config["kraken2"]["npartitions"]))
    resources:
