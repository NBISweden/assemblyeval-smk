rule kraken2_parallel:
    ...
    output:
        output = temp("{interim}/kraken2/{analysis}/{assembly}/{db}.{length}.{partition}.output.txt.gz"),
        unclassified = temp("{interim}/kraken2/{analysis}/{assembly}/{db}.{length}.{partition}.unclassified.fasta.gz"),
        report = temp("{interim}/kraken2/{analysis}/{assembly}/{db}.{length}.{partition}.report.txt"),
    input:
