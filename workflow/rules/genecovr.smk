...
rule genecovr_make_csv_inputfile:
    ...
    run:
        ...
        df = pd.concat([
            ...
            pd.Series([str(__INTERIM__ / f"gmap/map/{a}-{b}.psl") for a, b in itertools.product(assembly_keys, trx_keys)]),
            pd.Series([f"{a}" for a, b in itertools.product(assembly_fasta, trx_fasta)]),
            pd.Series([f"{b}" for a, b in itertools.product(assembly_fasta, trx_fasta)])],
                  axis=1)
