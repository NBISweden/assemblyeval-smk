...
rule blast_blastn_blastdb_parallel:
    ...
    envmodules:
        config["btk"]["blast"]["envmodules"],
    threads: config["btk"]["blast"]["threads"]
    shell:
...
rule seqtk_select_regions_bgzip:
    ...
    params:
        bed=lambda wildcards, input: os.path.join(
            os.path.dirname(input.done),
            wildcards.region,
            wildcards.prefix + ".fasta.bgz.bed",
        ),
    resources:
...
rule blastn_blast_partitions:
    ...
    input:
        ...
        out=expand(
            "{{interim}}/btk/{{blobdir}}/blastn/{region}/{{prefix}}.fasta.bgz.{{db}}.out.gz",
            region=[1 + p for p in range(config["btk"]["blast"]["npartitions"])],
        ),
    wildcard_constraints:
        prefix="[^/]+",
    resources:
